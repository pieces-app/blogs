...
e>$ mkdir ssl-cert<br>$ cd ssl-cert<br>$ mkdir Cert-Auth<br>$ cd Cert-Auth<br>$ openssl genrsa -out Cert-Auth.key -des3 2048</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=f07646b525" target="_blank" rel="noopener noreferrer">Save this code </a>
...
![](s-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb29875dd3e2_image8_aa6a18eb029e0065f7b1232e3a7c237b_800.p)
...
e>$ openssl req -x509 -sha256 -new -nodes -days 365 -key Cert-Auth.key -out Cert-Auth.pem</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=3709458e8d" target="_blank" rel="noopener noreferrer">Save this code</a>

In the command above, we used the already created ```certificate key``` to generate the ```certificate pem file```.

![](ttps://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb7a195dd3e5_image1_3aa7187441d595f39570563ad4ed8834_800.p)
...
e>$ mkdir localhost<br>$ cd localhost<br>$ touch localhost.ext</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=680f4f99ce" target="_blank" rel="noopener noreferrer">Save this code</a>
After creating the ```localhost.ext``` file, we will open the file and add the code snippets below to it.
e>authorityKeyIdentifier = keyid,issuer<br>basicConstraints = CA:FALSE<br>keyUsage = digitalSignature, nonRepudiation, keyEncipherment, dataEncipherment<br>subjectAltName = @alt_names<br>[alt_names]<br>DNS.1 = localhost<br>IP.1 = 127.0.0.1</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=c13849946a" target="_blank" rel="noopener noreferrer">Save this code</a>
...
![](c="https://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb817c5dd3e0_image2_7ff9c14e21783c29ffd16f220d7937e2_800.p)
...
e>$ openssl genrsa -out localhost.key -des3 2048</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=3507499d51" target="_blank" rel="noopener noreferrer">Save this code</a>
The commands above will generate a private key and ask for a simple passphrase. The user will enter and re-enter the passphrase for confirmation.
![](." src="https://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb5fb85dd3da_image3_2dad0306e648246120df0c6298e9565d_800.p)

Once you’ve done that, type the below command to generate the CSR:
e>$ openssl req -new -key localhost.key -out localhost.csr</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=6b874b8dff" target="_blank" rel="noopener noreferrer">Save this code</a>

The commands above will generate a CSR and ask for certain pieces of information. They will also request a challenge password. A &quot;<strong>challenge password</strong>&quot; is essentially an embedded, one-time shared secret between you and the SSL certificate issuer (CA) that the issuer may use to authenticate you should that ever be necessary.
Note the requested challenge password can be anything, but it should be a “<strong>strong</strong>” password.
![](webflow.com/6143afec68f555387049efb3/639357d878d1eb62405dd3e4_image7_fdc3c3a19e671ccaf877876f15723d9e_800.p)
...
e>$ openssl x509 -req -in localhost.csr -CA ../CA.pem -CAkey ../CA.key -CAcreateserial -days 365 -sha256 -extfile localhost.ext -out localhost.crt</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=1ed94bae94" target="_blank" rel="noopener noreferrer">Save this code</a>
...
![](-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb0cd35dd3de_image6_1454a74fd4d87cacb726008a93cb8237_800.p)

We will decrypt ```localhost.key``` since our ```localhost.key``` is in encrypted form. we will do this with the command below:
e>$ openssl rsa -in localhost.key -out localhost.decrypted.key</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=4129408a5d" target="_blank" rel="noopener noreferrer">Save this code</a>
...
e>$ mkdir ssl-nodejs<br>$ cd ssl-nodejs<br>$ npm init -y<br>$ npm i express<br>$ touch index.js</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=a46f4a9d1c" target="_blank" rel="noopener noreferrer">Save this code</a>
...
e>const express = require('express');<br>const app = express();<br><br>app.use(express.json())<br>app.get('/', (req, res)=&gt; {<br> res.send('hello world')<br>})<br>module.exports = app</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=65bb4ebe84" target="_blank" rel="noopener noreferrer">Save this code</a>

We will also import the Express app to our ```index.js file``` and use it to create the server.
e>const http = require('http');<br>const fs = require(&quot;fs&quot;)<br>const { getDesktopFolder } = require('platform-folders')<br>const path = require(&quot;path&quot;)<br>const https = require('https')<br>const app = require('./app')<br><br>const server = https.createServer({<br> key: fs.readFileSync(&#x60;${getDesktopFolder()}/ssl-cert/localhost/localhost.decrypted.key&#x60;),<br> cert: fs.readFileSync(&#x60;${getDesktopFolder()}/ssl-cert/localhost/localhost.crt&#x60;)<br>}, app)<br><br>server.listen(3000, ()=&gt; {<br> console.log('port running on', 3000)<br>})</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=b3c54b9fbd" target="_blank" rel="noopener noreferrer">Save this code</a>
...
e>$ node index.js</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=c37641a624" target="_blank" rel="noopener noreferrer">Save this code</a>
...
![](ttps://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb55935dd3dd_image12_7ba3c3ccc0331ac691d3f1cf46c687da_800.p)
...
![](s://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb35495dd3dc_image11_4085aa0f1e7e5f105af1bf593adfda7c_800.p)

<strong>N/B:</strong> This should not be done in production. However, we do this for development purposes in order to view the response on localhost from the browser.

![](ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb50d25dd3d9_image4_17c923c8adabc6387c6d36284f9cba46_800.p)
...
![](s general settings page." src="https://uploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1ebe6355dd3e3_image13_abc081ab60da78e10285cdb83d4b8a2b_800.p)

<strong>N/B:</strong> This is not advised to be done on production. However, we do this for development purposes in order to view the response of localhost on Postman.
Once this is done, we can access the HTTPS localhost on Postman.

![](ploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1eb6b3f5dd3df_image5_1246ddc340f94cfe5950a37beb0a6cb1_800.p)
...
e>brew install mkcert</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=5b8144b83f" target="_blank" rel="noopener noreferrer">Save this code</a>

![](ssl.webflow.com/6143afec68f555387049efb3/639357d878d1ebb7bb5dd3e1_image9_06c6b1a2a62e0bca7fb7b41ce8de6f9a_800.p)
...
e>mkcert localhost</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=52f24e9e3d" target="_blank" rel="noopener noreferrer">Save this code</a>
![](ploads-ssl.webflow.com/6143afec68f555387049efb3/639357d878d1ebcc485dd3db_image10_1f31ac9875d1d302cba47a2491215d90_800.p)

However, you can also chose to create a locally trusted CA by running the command:
e>mkcert -install</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=3823418852" target="_blank" rel="noopener noreferrer">Save this code</a>
...
e>mkcert -CAROOT</pre><p><a href="http:// https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=76b043a07c" target="_blank" rel="noopener noreferrer">Save this code</a>
...
e>const http = require('http');<br>const fs = require(&quot;fs&quot;)<br>const { getDesktopFolder } = require('platform-folders')<br>const path = require(&quot;path&quot;)<br>const https = require('https')<br>const app = require('./app')<br><br>const server = https.createServer({<br> key: fs.readFileSync(&#x60;${getDesktopFolder()}/mk-cert-ssl/localhost-key.pem&#x60;),<br> cert: fs.readFileSync(&#x60;${getDesktopFolder()}/mk-cert-ssl/localhost.pem&#x60;)<br>}, app)<br><br>server.listen(3000, ()=&gt; {<br> console.log('port running on', 3000)<br>})</pre><p><a href="https://user-648f958b-2a5e-46c0-b5b3-819f06744dab-agyqaaz4hq-uc.a.run.app/?p=9fb94a8797" target="_blank" rel="noopener noreferrer">Save this code</a>
<strong>Configure the Chrome web browser and the Postman API client to allow certificates we have signed as the CA</strong>
Since we have already configured the web browser to view self-signed certificates, we can easily view the result below:

![](.webflow.com/6143afec68f555387049efb3/639357d878d1eb50d25dd3d9_image4_17c923c8adabc6387c6d36284f9cba46_800.p)
