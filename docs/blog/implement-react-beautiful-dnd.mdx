...
![](utiful DnD." src="https://uploads-ssl.webflow.com/6143afec68f555387049efb3/633d9f691db56726cbfbe5a2_image1_df7be4ab1983a48fb63217626b49cabb_800.p)
...
e>npx create-react-app drag-n-drop-demo</pre><p>Then, install the dependencies:
First, let’s make a simple list of tasks as a starting point:
e>const initialTasks = [<br>  {<br>    id: 1,<br>    title: &quot;Task 1&quot;,<br>  },<br>  {<br>    id: 2,<br>    title: &quot;Task 2&quot;,<br>  },<br>  {<br>    id: 3,<br>    title: &quot;Task 3&quot;,<br>  },<br>];</pre><p><a href="https://a5411705-e582-4f98-a138-5ded95573d50.pieces.cloud/?p=048f4f9d14" target="_blank" rel="noopener noreferrer">Save this code</a> 
...
e>&lt;DragDropContext onDragEnd={onDragEnd}&gt;<br>  All drag-and-drop functionality happens inside this context<br>&lt;/DragDropContext&gt;</pre><p>You’ll notice a special function called ```onDragEnd``` passed into the context. We’ll use this function to update the state of our application.
For now, let’s use the following code:
e>const onDragEnd = (result) =&gt; {<br> console.log(result);<br>};</pre><p>Basically, this function will be called after a user finishes a drag and drop operation.
...
e>const onDragEnd = (result) =&gt; {<br> if (!result.destination) return;<br><br> const items = Array.from(tasks);<br> const [reorderedItem] = items.splice(result.source.index, 1);<br>  items.splice(result.destination.index, 0, reorderedItem);<br><br>  setTasks(items);<br>};</pre><p><a href="https://a5411705-e582-4f98-a138-5ded95573d50.pieces.cloud/?p=870142bf32" target="_blank" rel="noopener noreferrer">Save this code</a> 
...
Instead of this:
e>ReactDOM.render(<br>  &lt;React.StrictMode&gt;<br>    &lt;App /&gt;<br>  &lt;/React.StrictMode&gt;,<br>  document.getElementById(&quot;root&quot;)<br>);</pre><p>Use this:
e>ReactDOM.render(&lt;App /&gt;, document.getElementById(&quot;root&quot;));</pre><p>This should solve the issue.
...
e>.react-beautiful-dnd-draggable {<br>  transition: transform 0.2s;<br>}</pre><p>3. The draggable item is not moving to the correct position.
