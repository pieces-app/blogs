...
## <strong>The Intersection Observer vs Other Observer APIs</strong>
...
e>let options = {<br>  root: document.querySelector('#scrollArea'),<br>  rootMargin: '0px',<br>  threshold: 1.0<br>}<br>let observer = new IntersectionObserver(callback, options);</pre><p>From the above, a threshold of 1.0 entails that when 100% of the target is visible within the element specified by the root option, the callback is invoked.
